# Tutorials

The emodpy-hiv tutorials introduce you to the key components of configuring, running, and analyzing EMOD-HIV simulations with Python. Each tutorial builds on the previous one, starting from a simple working example and progressively adding features.

| Tutorial | Description |
|----------|-------------|
| [Tutorial 1: Run a simulation](tutorial-1.md) | Run a baseline Zambia simulation using the built-in country model |
| [Tutorial 2: Add reports and plot results](tutorial-2.md) | Add output reports, download results, and plot simulation data |
| [Tutorial 3: Customize with build functions](tutorial-3.md) | Write your own build functions to modify the baseline country model |
| [Tutorial 4: Override the cascade of care](tutorial-4.md) | Subclass a country model to customize the HIV cascade of care |

Before running the tutorials, see [Setup](setup.md) to prepare your environment.
