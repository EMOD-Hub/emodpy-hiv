...
The simulation statistics report (ReportSimulationStats.csv) tracks performance and resource usage
data for each time step of the simulation. It records wall-clock timing, process memory, system
memory, and counts of key simulation objects (individuals, infections, interventions, and events).
This report is useful for diagnosing performance bottlenecks, monitoring memory growth over the
course of a simulation, and profiling how simulation size changes over time.
