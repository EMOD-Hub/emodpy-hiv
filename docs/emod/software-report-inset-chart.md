...
| Report_HIV_Event_Channels_List | array of strings | NA | NA | [] | The list of events to be included in the report. For each event specified with this parameter, one channel will be added to the report. See [Event list](parameter-campaign-event-list.md) for possible values. If no events are listed, a 'Number of Events' channel will be included on the report and display the total number of all events that occurred during the simulation. |
